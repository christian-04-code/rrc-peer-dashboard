...
- **Latest commit**: "UI polish: cleaner Guidance drawer, chart point tooltips, Bear/Base/Bull preset explainer"
...
## Validation Results
- `npm test`: 143/143 pass (127 prior + 16 new across `tests/guidance-panel.test.cjs` (+3), `tests/chart-tooltip.test.cjs` (new, 5), `tests/rrc-scenario-preset-tooltip.test.cjs` (new, 4))
- `npm run typecheck`: clean
- `npm run build`: succeeds (all routes compile, including `/forecast` and `/api/rrc-scenarios`)
- Manually verified in-browser (dev server + DOM inspection, since the automated screenshot tool didn't reliably render the `position:fixed` drawer/native mouse-hover overlays): guidance drawer opens with all 8 RRC sections structured into label/value rows + notes; chart tooltip shows ticker/period/value/unit and a "MODELED" badge on forecast points; Bear/Base/Bull info tooltip shows the exact three assumptions per preset.
