...
## MACRO PHASE 2 -- PEER ANALYTICS FIX, DIRECTIONAL COLORS, REGION-FIRST STORAGE MAP, GAS NEWS, RISK DE-DUP (2026-09-22, later same day) — READ THIS FIRST

This section supersedes the "MACRO DASHBOARD PRODUCTION REPAIR + UI CLEANUP" section directly below it (still accurate for its own commits, kept for history) for Macro/Peer-Analytics purposes. Everything further below is the older, unrelated IR Report Enhancement workstream.

### 1. Production state

- **Final commit this pass:** `fcc43ec` (plus the two earlier same-day commits `60957d3` diagnostic logging and `73e6fd4`/`955907a` content-budget fixes described in §2 below).
- **Production URL:** https://rrc-peer-dashboard.vercel.app — confirmed deployed and matching via `vercel inspect --logs`.
- **Working tree:** clean at handoff time.
- **Test/build status (same commit):** `npx tsc --noEmit` clean; `npm test` — **1447 pass, 0 fail, 81 skipped** (DB-gated, expected, no local Postgres); `npm run build` clean.
- **Known transient condition at handoff time (NOT a regression, self-resolving):** EIA's public API is currently rate-limiting (`http_429`) this project's `EIA_API_KEY` on the regional-storage route, almost certainly from this session's own cumulative testing volume (repeated `/api/macro` fetches, manual cron triggers, local dev server). The retry/backoff logic (from the morning's pass) is working exactly as designed — 4 attempts, then an honest `"unavailable"`/`"--"` rather than a fabricated value. Confirmed via direct Vercel log inspection. Re-check `/api/macro`'s `storage.regions.*.status` after some time has passed; if still `"unavailable"` after, say, an hour, investigate the EIA key's actual rate-limit quota, but do NOT assume the retry code itself is broken -- it was unit-tested and was working with real data earlier in this same session (verified 795/934/1039/241/289 Bcf across the 5 regions before this rate-limit condition appeared).

### 2. Peer Analytics -- NOW FIXED AND VERIFIED (was the top-priority open item; do not reopen without new evidence)

Confirmed end-to-end, not just "code looks right":
- Root cause #1 (from the morning's pass): EIA fetch retry/backoff -- already fixed, commit `31ec62d`.
- **Root cause #2 (found and fixed THIS pass, previously undiscovered because the pipeline had never gotten far enough to hit it):** PDF rendering itself was overflowing the 5-page hard cap. The real week-ending 2026-09-11 report's `STANDARD` render produced 7 pages and its one allowed `REDUCED` retry also produced 7 (later 6, still over) -- confirmed via a manually triggered production cron run (`vercel crons run /api/cron/reports`) returning `{"stage":"failed","reason":"Reduced-content render still produced 7 pages...","publishStatus":"render_failed"}`. Traced to the IR-report-enhancement content (investor questions, guidance watch, valuation, company/peer news evidence sections -- commit `ab5b10f`, already merged to main well before today, NOT something introduced this session) never having been budget-tested against a real full week's worth of that content, since the pipeline had been stuck on stale 2026-08-28 data for ~4 weeks and never actually exercised this code path for real until today.
- **Fix:** `lib/reports/render/content-budget.ts`'s `REDUCED_BUDGET` tightened in two iterations (commits `955907a`, `73e6fd4`) -- verified locally each time by rendering the existing `tests/fixtures/weekly-report-fixture.ts` sample (both as-is and augmented with a realistic full load of IR-enhancement content: 6 investor questions, extra guidance/news items with high materiality) through a REAL local Chromium instance (`puppeteer-core` + system Chrome, not `@sparticuz/chromium`'s Lambda-tuned launch args, which crash a desktop browser -- see the throwaway measurement script's own comments if you need to reproduce this locally). `STANDARD_BUDGET` and all classification/ranking/AI logic were NOT touched.
- **Also added (commit `60957d3`, kept permanently):** `app/api/cron/reports/route.ts` now `console.log`s the full orchestration result (`stage`/`reason`/etc.) -- this was previously completely invisible in Vercel logs (the route deliberately never leaks it to the HTTP response body), which is why this bug took real production log archaeology, not just code review, to find. Next agent debugging a future stuck-report scenario: check this log line FIRST via `vercel logs` before anything else.
- **Confirmed published:** `storageWeekEnding: "2026-09-11"`, `publishedAt: "2026-09-22T19:17:38.297Z"`, a real 5-page, 173,230-byte PDF (downloaded and independently page-counted, not just trusted from the API response). Live Overview page confirmed via browser automation showing "Week Ending Sep 11, 2026" and a working download link pointing at `/api/reports/latest/download`.
- **If the report goes stale again in the future:** don't assume it's the same root cause. Check `/api/cron/reports`'s own log line first (see above) for the actual `stage`/`reason` -- it could be a THIRD, still-undiscovered issue. `vercel crons run /api/cron/reports` is the correct, legitimate way to manually trigger a stuck cron for diagnosis (official Vercel CLI feature, not a credential workaround) -- prefer it over trying to obtain `CRON_SECRET` some other way.

### 3. Directional color system (Phase 2 of this pass's brief) -- implemented

- New `directionClass()` helper in both `MacroPanel.tsx` and `MacroEnergyMap.tsx` (small, intentionally duplicated rather than cross-imported -- one line of logic each): `null`/`undefined`/`0` → no color (neutral), positive → `.positive`, negative → `.negative`. Reuses the pre-existing global `.positive`/`.negative`/`.neutral` CSS classes (`app/globals.css`) -- no new palette.
- Applied to the `Stat` component (Gas Balance's 6 cards, Appalachia's 6 cards + YoY/MoM, and every other YoY/change Stat across Supply/LNG/Demand/Propane) via a new `direction`/`noteDirection` prop pair -- absolute-value Stat usages (e.g. "Latest native observation", "5-year same-week range") were deliberately left without a `direction` prop and remain neutral.
- Regional Storage Table (`RegionalStorageTable` in `MacroVisuals.tsx`): Weekly Δ / vs YA / vs 5Y columns now colored by sign; Current/Prior Wk/Year Ago/5Y Avg (absolute) columns untouched. Header row changed from the cyan source-accent to bold white (explicit request) -- `tests/macro-fundamentals.test.cjs` updated accordingly (old test asserted the literal `#75c7ee`).
- Macro Snapshot: the pre-existing `tone: "positive"|"neutral"|"negative"` classification (`lib/market/macro-analytics.ts`'s `toneFor()`) was **not modified** -- it already correctly mapped Balanced/Near Normal/Stable/Neutral/Unavailable to `"neutral"`. Only added CSS: `.macro-snapshot-item.neutral summary strong { color: var(--caution); }` (reusing the existing amber `--caution` token already used for MODERATE_RISK elsewhere, not a new color).
- Storage map detail panel (`MacroEnergyMap.tsx`): Weekly change / vs year ago / vs 5-year average (storage) and Month over month / Year over year (production) now colored; absolute "Regional working gas"/"Latest production" stayed neutral.
- **Known judgment call, not re-litigated:** "Unavailable" states share the same neutral/amber treatment as genuinely balanced ones (both previously had zero distinct styling, so this isn't a regression) -- if a future pass wants "Unavailable" to look visually distinct from "Balanced", that needs a new CSS hook (e.g. a class or data attribute keyed off the literal state text, since `tone` alone conflates them), not a classification change.

### 4. Storage map region-first UX (Phase 3, the biggest pending item from the morning's handoff) -- implemented and verified

- Clicking any state in **Storage** mode now highlights **every state sharing its EIA storage region** (not just the clicked state) -- `isSelected` in `MacroEnergyMap.tsx`'s state-path rendering branches on `mode`. **Production mode is a completely separate code path and still highlights only the exact clicked state** -- verified live via browser automation (Storage→click Illinois→all 9 Midwest states highlighted; switch to Production→only the previously-selected state highlighted; switch back to Storage→region highlighting resumes correctly).
- Detail panel (`aside.macro-map-detail`) in Storage mode now leads with `"<Region> Storage Region"` (e.g. "Midwest Storage Region") as the `<h3>`, with `"Selected state: <State>"` as a secondary line below it -- confirmed live. Production mode's `<h3>{selectedName}</h3>` (state name, primary) is completely unchanged.
- The long always-visible caveat sentence ("EIA reports weekly storage for the X region—not separately for Y.") moved into a reused `DataInfoTooltip` (no third tooltip system).
- `lib/market/storage-regions.ts`'s `REGION_STATES` mapping was **read but not modified** -- per the prior handoff's own explicit caveat, it still has not been independently verified against EIA's current official region-definition publication; that remains a genuinely open item (see §6).
- New test: `tests/macro-fundamentals.test.cjs` — "storage map selection is region-first" — asserts the region-wide-highlight condition, the production state-only condition, and both detail-panel heading forms are present in source.

### 5. Layout/whitespace fixes (Phase 4)

- **A. Gas Balance card empty space → filled with real content, not invented filler.** New `lib/news/select-featured-story.ts` (`selectFeaturedNaturalGasStory`) picks the most recent `category: "natural_gas"` article with `processingStatus` `"retained"` or `"analyzed"` from the EXISTING News pipeline (`useNewsArticles()` hook, already-persisted/analyzed articles, no new fetch/API/AI call). Renders as "LATEST NATURAL GAS NEWS" with real headline/publisher/date and a real outbound link; clean italic "No current natural gas story available." empty state when none qualify. Verified live: real headline "Physical Natural Gas Prices Peter Out as Weather Cools, Maintenance Trims LNG Volumes" (Natural Gas Intelligence, Sep 21, 2026) rendered correctly, both desktop (1600px) and mobile (500px) widths, no layout breakage.
- **B. Weekly Storage card's dead bottom space -- root-caused and fixed.** `.macro-balance-grid` (the chart | aside row) had no `align-items` override, so the default `stretch` inflated the shorter `.macro-weekly-report` aside to match the much-taller storage chart, leaving its own internal `align-content: start` content stranded at the top with a large empty bordered area below. Added `align-items: start`. Verified live -- no more dead space; the aside is now simply its own natural (shorter) height beside the chart, which is the expected/correct outcome per this project's own established "a modest natural height difference between differently-shaped cards is not a bug" principle.
- **C. Lower-48 storage chart letterboxing -- root-caused and fixed.** `.macro-storage-chart` had its own separate flat `height: 220px` (never inherited the earlier session's `.macro-evidence-chart` `aspect-ratio` fix, because it uses its own class, not the shared wrapper -- exactly the gap flagged in the prior handoff's §5B). Changed to `height: auto; aspect-ratio: 660 / 220`. Verified live -- chart now fills the full card width.
- **D. "EIA Outlook large right-side empty space" -- investigated, NOT reproducible in the current build, no change made.** Spent real effort on this: measured both plausible targets (the Demand card's `.macro-structural-outlook` mini-panel, and the actual `#eia-outlook` section's chart) via DOM geometry AND screenshots at a genuine 1600px-wide production render. Both fill their containers edge-to-edge with no blank void. The one real asymmetry still visible is that the Demand card (3 chart subsections) is naturally much taller than its NGL sibling in `.macro-grid-row-demand` -- but that row already has a deliberately weighted `7fr : 5fr` column split AND `align-items: start` (both pre-existing from an earlier session), which is already the correct structural mitigation; forcing it further would mean either fabricating NGL content (forbidden) or moving sections between cards (a restructure, out of scope for this pass). Recommend closing this item as "already resolved by earlier chart-sizing work" unless a future screenshot shows a genuinely different symptom.

### 6. Biggest Risks & Opportunities de-duplication (Phase 5) -- implemented

- `lib/market/macro-risk-engine.ts`: every one of the 7 driver signals now exposes both `reason` (**byte-for-byte unchanged** -- it is also `MacroRiskPayloadSignal.deterministicReason`, the exact string persisted/fingerprinted for the AI provider, and must never drift) and a new `interpretation` field (same deterministic per-driver qualifier-clause template, just not re-joined with the evidence-restatement clause that duplicated the metrics row above it). Verified via exact-string-equality tests that `reason` did not change for any driver, including the two with a trailing STEO-forecast-direction sentence (LNG, Industrial) and the `UNAVAILABLE` case.
- `MacroRiskWidget.tsx`'s driver cards now render `signal.interpretation` instead of `signal.reason`. Ranking, classification thresholds, `qualifier()` phrase text, and the AI/provider layer were **not touched**. Verified live: e.g. Storage card now reads "Working gas storage: 3,298 Bcf · vs 5-year average: +2.5%" (metrics row) → "Close to normal, not a clear price signal on its own." (interpretation only) instead of repeating "Storage is +2.5% versus its five-year average, ...".

### 7. Files changed this pass

`app/api/cron/reports/route.ts`, `lib/reports/render/content-budget.ts`, `app/globals.css`, `components/dashboard/MacroPanel.tsx`, `components/dashboard/MacroEnergyMap.tsx`, `components/dashboard/MacroVisuals.tsx`, `components/dashboard/MacroRiskWidget.tsx`, `lib/market/macro-risk-engine.ts`, new `lib/news/select-featured-story.ts`, plus test updates/additions in `tests/macro-analytics.test.cjs`, `tests/macro-fundamentals.test.cjs`, `tests/macro-risk-engine.test.cjs`, `tests/macro-risk-widget.test.cjs`, new `tests/news-select-featured-story.test.cjs`.

### 8. Remaining open items for the next agent

1. **EIA region definitions still not independently verified against EIA's official publication** (`lib/market/storage-regions.ts`) -- carried over from the prior handoff, still not done, still a genuine (if low-probability) data-integrity risk if ever touched carelessly.
2. **The transient EIA rate-limit condition described in §1** -- almost certainly self-resolving, but confirm regional storage data (`/api/macro`) is showing real values again before doing any further Storage-map work.
3. Phase 4D (EIA Outlook whitespace) -- see §5D; recommend treating as resolved/non-reproducible rather than reopening blindly.
4. Baker Hughes automation -- still explicitly not to be attempted without sign-off (unchanged from prior handoff).
5. No new work was started on anything not listed above -- this pass's scope was exactly Peer Analytics verification + Phases 2-5 of the UI/UX brief, per the user's own instructions for this session.

---

## MACRO DASHBOARD PRODUCTION REPAIR + UI CLEANUP (2026-09-22) — historical; superseded above for Macro/Peer-Analytics purposes, kept for its own commits' history
...
### 1. Security/config blockers (Issue #14 items 1-2)

- Removed `app/api/diag/cron-secret/route.ts` (the temporary Phase 7F diagnostic route with its own hardcoded, throwaway `x-diag-token` gate). It never exposed `CRON_SECRET` itself (only non-reversible SHA-256 fingerprints), so there was nothing to rotate once deleted -- confirmed the route table no longer lists `/api/diag/*` after a clean rebuild.
- **Confirmed still unresolved (owner action required): `CRON_SECRET` still has NO Production scope** -- `vercel env ls` shows it scoped only to `Preview (feat/daily-energy-intelligence)`. This means Production's own `/api/cron/macro` and `/api/cron/news` will fail Unauthorized on their next real invocation. This is a genuine, currently-live problem, not merely a future risk. Not touched this session (Production env changes are the owner's own action); flagged as a **hard release blocker**.

### 2. SEC-native financial sourcing (Issue #14 items 6, 14)

Audited `lib/dashboard/financials-quarterly.ts`'s existing "codex" extraction (already filing-derived per its own header -- built from each company's actual 10-Q/10-K/earnings materials, not invented) and the existing offline SEC XBRL pipeline (`lib/sec-pipeline/xbrl.mjs`, `scripts/sec/*.mjs`).

- Independently cross-checked Q2 2026 **revenue** for all 7 tracked tickers against the live SEC EDGAR/XBRL company-facts API (`data.sec.gov/api/xbrl/companyconcept`). **6 of 7 matched exactly, byte-for-byte, against the existing fixture value**: RRC ($833,571,000, accession 0001193125-26-310446), AR ($1,559,842,000, 0001104659-26-088153), CNX ($618,484,000, 0001070412-26-000058), CRK ($353,282,000, 0001193125-26-326260), EQT ($1,809,940,000, 0000033213-26-000043), GPOR ($323,228,000, 0001628280-26-052313). These 6 cells' `source` tag was upgraded from `"codex"` to a new `"sec-xbrl"` tag, with the note field recording the concept/accession/filing-date/form -- **the value itself never changed** (it already matched); only the provenance label and citation detail were upgraded, and only after independent verification, per the explicit "do not merely rename a source label without verifying its underlying provenance" instruction.
- **EXE (Expand Energy) could not be verified**: its Q2 2026 10-Q (filed 2026-07-28, confirmed to exist on EDGAR) had not yet been indexed into SEC's XBRL companyfacts API as of the verification date (2026-09-08) -- zero facts with that filing's accession number exist in the API response at all. Left as `"codex"` with this specific limitation documented in its own note, rather than claiming a verification that didn't occur.
- Non-GAAP/operating metrics (Adjusted EBITDAX, FCF, production, unit costs, capex-as-defined, net debt) are **not** standardized XBRL concepts and were correctly left as `"codex"`/`"sec-direct"`/`"factset"`, per their own already-documented definitions.
- **New: `lib/reports/adapters/source-labels.ts`** -- a shared, honest label mapping (`describeFinancialSource`/`describeMarketDataSource`) translating internal tags into real provenance descriptions ("SEC EDGAR/XBRL company facts API (verified)", "Company SEC filings & earnings materials (extracted)", "SEC EDGAR filing (Form 10-Q/10-K/8-K, direct extraction)", "FactSet (third-party financial data vendor)", "Macrotrends/Yahoo Finance/Nasdaq historical (market data)") -- never a raw tool-name tag. Wired into `range-company-adapter.ts` and `peers-adapter.ts` (which previously carried no per-item source metadata at all).
- Fixed `buildPeerComparisonTable`'s `sourceLine` (table-builder.ts): it previously read `${rangeItems[0]?.metadata.source}; peer quarterly financials` -- i.e. it always displayed whichever `range_company` item happened to be array-index 0's source, regardless of what the table's own columns were actually sourced from. A real published PDF showed this as literally `"codex (actual); peer quarterly financials"` on every render. Now composed from the actual, deduplicated per-column/per-company sources shown in that specific table.
- Manifest labels ("RRC quarterly financials (Codex/FactSet/SEC-direct extraction)", "Peer quarterly financials (Codex/FactSet/SEC-direct extraction)") reworded to name the real underlying materials instead of the internal tool codename.
- **Remaining limitation**: this pass verified only the specific values that actually reach the weekly report (Q2 2026 revenue, 7 tickers). It did not attempt to re-verify the entire multi-year, multi-metric fixture -- that remains "codex"-sourced, filing-derived-but-not-individually-re-verified data, honestly labeled as such.

### 3. News integration (Issue #14 item 15)

News evidence was already reused read-only from persisted, analyzed articles (`news-adapter.ts`) -- no re-fetch, no re-analysis, confirmed unchanged.

- `buildNewsTable` (table-builder.ts) expanded from 3 columns (Headline/Date/Range Impact) to 5 (Headline/**Publisher**/Date/**Category**/Range Impact), and the Range Impact cell now combines News's own persisted direction + strength (e.g. "moderate positive") instead of just direction.
- The Material News section's commentary (evidence-sections.ts) previously read only a generic "N items were retained" sentence with no actual news content. Now shows the single most material article's own persisted excerpt (a reported fact) as plain commentary, with the existing `RANGE IMPLICATION` callout (the system's interpretation, using News's own persisted `rangeImpactDirection`/`rangeImpactStrength` -- never re-derived) kept visually distinct, satisfying the "distinguish reported fact from interpretation" requirement.

### 4. Report content/quality fixes -- all independently reproduced against the real published PDF

- **"$3.00/Mcf forecast floor" (Issue #14 item 3)**: confirmed present verbatim on page 1 of the real PDF (and inconsistently called a "forecast midpoint" on page 4 of the same report). Root cause: the AI system prompt had no rule against characterizing a forecast as a threshold. Fixed at two layers: (a) `lib/reports/ai/prompt.ts` rule 13 explicitly forbids "floor/ceiling/support level/resistance level/investment threshold" framing and requires keeping spot/realized/forecast price terms distinct; (b) `lib/reports/ai-contract.ts` added a new denylist, `FORECAST_OVERCLAIM_PATTERNS`, as a hard code-level backstop (same "small denylist" pattern as the existing `GUARANTEED_LANGUAGE_PATTERNS`) -- a response using this language is now rejected and retried, not merely discouraged.
- **"storage normality... no meaningful downward pressure" stated as fact (item 4)**: confirmed verbatim in `biggestOpportunity`, a field the existing `checkGuardedText` filler/guaranteed-language check never covered at all (only `executiveAssessment`/`bottomLine` were checked). Fixed: (a) prompt rule 14 requires hedged language ("typically," "historically") for directional relationships and forbids "imposes no pressure"/"normal"/"benign" framing; (b) `checkGuardedText` is now also applied to every narrative item (`biggestRisk`/`biggestOpportunity`/`whatChanged[]`) and every `managementWatchItems[].reason`, closing the coverage gap.
- **First-report "What Changed" flooding (item 5)**: root-caused to `lib/reports/changes.ts`'s `computeWeeklyChanges()` -- when `previousModules === null` (the true first-ever report, no baseline exists), it treated every single current evidence item as an individually "new" change, producing dozens of records the AI then had to compress into a confusing "framework initialized" narrative. This reverses a deliberate Phase 7B decision (see the function's own updated header comment for the full reasoning) now that a real report showed the actual reader-facing consequence. Fixed: returns `[]` for the true-first-report case; a real prior snapshot with a merely-empty category (`{}`) is unaffected and still reports genuinely new items. The existing "no previous published report exists" prompt disclosure already handles this correctly once `whatChanged` is actually empty.
- **Missing directional arrows (item 7)**: confirmed the real PDF rendered `(   72.7% QoQ)` with a visible blank gap where an arrow glyph should be, on every single comparison annotation, not just one. Root cause: `@sparticuz/chromium`'s bundled fonts on Vercel's serverless Linux runtime don't cover the ↑/↓/→ Unicode code points used in `table-builder.ts`/`evidence-sections.ts`. Fixed by replacing every arrow with a leading ASCII sign (new shared `lib/reports/render/format.ts`, `formatSignedPct()`) -- "+3.6%"/"-72.7%"/"flat" -- guaranteed-renderable and, as a side effect, directly supplies the "minus sign for a decline" the review separately asked for.
- **"$-211MM" instead of "-$211MM" (item 7)**: the three duplicated `moneyDisplay()` functions (range-company-adapter.ts/peers-adapter.ts/forecast-adapter.ts) put the sign after the currency symbol. Consolidated into one shared `lib/reports/adapters/format.ts`, fixed to standard convention.
- **Five items in a six-cell "At a Glance" grid (item 7)**: the CSS used a fixed 3-column grid, which leaves a visible empty background-colored trailing cell whenever the item count isn't a multiple of 3 (the common case). Changed to `flex-wrap` so a non-multiple-of-3 count never leaves a dangling cell.
- **Peer table truncated to 5 of 7 companies, sources table hiding 6+ of ~14 rows, and 7 of 11 evidence-section candidates omitted while page 5 sat mostly empty (items 7-9)**: root-caused to `content-budget.ts`'s `REDUCED_BUDGET` (the tier the actual first-ever report fell back to) being tuned far more conservatively than the real per-item vertical space required. Raised `maxSourceRows` to 20 for both tiers (a source row is one compact line -- effectively never truncated in practice now), `REDUCED_BUDGET.maxPeerCompanies` to 6 (all tracked peers -- a peer-table row is cheap), and `REDUCED_BUDGET.maxEvidenceSections` from 4 to 5. Re-validated against the real render pipeline (not guessed): the project's own sample fixture renders to exactly 5 pages under `STANDARD_BUDGET` and 4 under `REDUCED_BUDGET` with the new numbers -- still within the hard 5-page maximum. Some residual trailing white space on a genuinely quiet week is an inherent, honestly-disclosed consequence of a fixed page cap with variable content, not a bug to compress away (compressing further would mean smaller fonts or dropped evidence, which the brief explicitly forbids).

### 5. Report branding rename (Issue #14 item 13)

Removed "Intelligence" from every user-facing surface of the PDF itself (the Overview UI control was already renamed to "Weekly AI Report" in an earlier session pass): masthead title ("WEEKLY RANGE RESOURCES AI REPORT"), subtitle ("Market, Company & Peer Analysis"), the body's "Executive Assessment" section heading (was "Weekly Range Resources Intelligence Assessment"), the Puppeteer footer template, and the download filename (`Weekly-Range-Resources-AI-Report-{week}.pdf`). "AI" is kept everywhere for attribution transparency, per the explicit instruction not to remove it. This supersedes the earlier Phase 7F-era instruction to keep the PDF's own title unchanged -- Issue #14 is the more recent, explicit product decision.

### 6. Validation

Targeted tests for every changed subsystem (211 tests) + full suite: **1465 tests, 1384 pass, 0 fail, 81 skipped** (all pre-existing DB-gated tests, unchanged standing limitation -- no new skips introduced). `npm run typecheck`: clean. `npm run build`: clean; route table confirms `/api/diag/*` is gone and no new routes were added. All fixes re-verified against the real render pipeline locally (sample fixture rendered through the actual `html-template.ts` + headless Chrome, not just unit-asserted) before being considered done. The already-published report (storage week 2026-08-28) was never touched, re-generated, or re-published this pass.

### 7. Release status

**Not ready for Production merge/deploy.** Two independent, unresolved blockers:
1. `CRON_SECRET` has no Production scope (section 1 above) -- a real, currently-live problem for existing Production crons, not merely a future risk.
2. PR #13 is still marked **draft** on GitHub, is 52 commits / 27k+ lines ahead of `main` with a stale Phase-5.1-era description, and its own body explicitly says "Keep this PR DRAFT for now. Do not merge automatically." Per the user's own standing instruction ("if a blocker remains, stop and report it rather than forcing the release"), this session did not mark it ready for review, update its description, or attempt a merge -- that requires the user's own fresh confirmation given both the scale of the diff and the PR's own explicit prior instruction.

---

## PHASE 7F PREVIEW VALIDATION IN PROGRESS (2026-09-03) — historical; superseded above
