...
## 25. What remains — Phase 7F through 7G (not started; not authorized by this phase)
...
- **7F — Orchestration**: the actual gated cron (§17) — new `vercel.json` entry, `app/api/cron/reports/route.ts`, advisory-lock-guarded orchestration function that calls `runWeeklySnapshotBuild()`, `generateWeeklyAnalysisIfNeeded()` (§26.9), then `publishWeeklyReportIfReady()` (§30.1, which itself calls `renderWeeklyReportPdf()`, §29.6) to carry a `ready` snapshot + assessment all the way to `published` in one scheduled run. **Important implementation note carried forward from §29.10/§30.9: this route MUST add its own `next.config.js` `outputFileTracingIncludes` entry (keyed to its own path) for `@sparticuz/chromium`'s `bin/**` assets, or the exact packaging failure 7D.2 diagnosed and fixed will recur for this route.** Also where the first real (paid) Anthropic call and the first real scheduled Chromium invocation for this subsystem should happen, via the same controlled-Preview-validation pattern Phase 6/7D.2 already established.
...
### 30.9 What remains for Phase 7F

- **The actual trigger.** Nothing calls `publishWeeklyReportIfReady()` yet -- Phase 7F's orchestration route is what will call `runWeeklySnapshotBuild()` -> `generateWeeklyAnalysisIfNeeded()` -> `publishWeeklyReportIfReady()` in sequence, on a schedule, with real readiness/timing gating (the 60-minute post-EIA-storage buffer, etc.).
- **The Phase 7D.2 packaging requirement carries forward unchanged**: that future route (wherever it invokes `ChromiumPdfRenderer`, whether directly or via `publishWeeklyReportIfReady()`) **must** add its own `next.config.js` `experimental.outputFileTracingIncludes` entry, keyed to its own route path, force-including `./node_modules/@sparticuz/chromium/bin/**` -- `serverComponentsExternalPackages: ["@sparticuz/chromium"]` alone is not sufficient, confirmed live in Phase 7D.2 (§29.10). This is the single most important implementation note left for Phase 7F.
- **The real first live Anthropic call and the real first live Chromium invocation from a scheduled path** both still belong to Phase 7F, via the same controlled-Preview-validation pattern Phase 6/7D.2 already established.
