...
- **7F — Orchestration**: the actual gated cron (§17) — new `vercel.json` entry, `app/api/cron/reports/route.ts`, advisory-lock-guarded orchestration function that calls `runWeeklySnapshotBuild()`, `generateWeeklyAnalysisIfNeeded()` (§26.9), then `renderWeeklyReportPdf()` (§29.6) and (once 7E exists) carries a `ready` snapshot + assessment + PDF the rest of the way to `published`. **Important implementation note carried forward from §29.10: this route MUST add its own `next.config.js` `outputFileTracingIncludes` entry (keyed to its own path) for `@sparticuz/chromium`'s `bin/**` assets, or the exact packaging failure 7D.2 diagnosed and fixed will recur for this route.** Also where the first real (paid) Anthropic call for this subsystem should happen, via the same controlled-Preview-validation pattern Phase 6 established (§26.10).
