...
### 31.4 The data-driven safety buffer (`PUBLISH_SAFETY_BUFFER_MS`, 1 hour)

The brief called for "roughly a one-hour safety buffer" after the newest EIA storage observation is ingested/validated, but explicitly forbade brittle wall-clock dependence. The implementation checks elapsed real time against the snapshot's own already-existing `createdAt` timestamp (frozen the moment `runWeeklySnapshotBuild()` first reaches `"ready"` for that week) rather than against any cron-schedule assumption:

```ts
function bufferRemainingMs(snapshot, now) {
  const elapsed = now.getTime() - new Date(snapshot.createdAt).getTime();
  return Math.max(0, PUBLISH_SAFETY_BUFFER_MS - elapsed);
}
```

Because `getActiveSnapshotForWeek` (Phase 7A/7B, unchanged) returns the *same* row with its *original* `createdAt` on every subsequent call for an already-`ready`-but-unpublished week (via `runWeeklySnapshotBuild()`'s own `active_attempt_exists` status), this buffer check requires zero new persistence and zero new state -- it is a pure read of data that already existed. Practical consequence, accepted explicitly per the brief: since Hobby cron only runs once/day and isn't guaranteed to the minute, a snapshot first detected on a Thursday will typically clear its one-hour buffer by the *time the buffer check next runs* -- which, given the 17:00 UTC schedule, is usually the **next day's** run, not a same-day retry an hour later. Correctness (never publishing before the buffer has genuinely elapsed) is guaranteed; hitting exactly one hour after release is not, and was never required to be.
...
New file `tests/weekly-report-orchestrate-weekly.test.cjs` (26 tests, all pass, none DB-gated): the full happy-path publish (with an exact provider-call-count assertion of 1); an `active_attempt_exists` reused-snapshot path that still reaches `published`; already-published no-op; not-ready for a missing valid storage period, for unmet readiness, and for the safety buffer (including an exact-boundary case one second short of the full hour); `locked` for an unavailable advisory lock (with a `buildSnapshot` fake that throws if called, proving it's never reached) and for an in-progress concurrent snapshot attempt; not-ready (not failed) for each of `ANTHROPIC_API_KEY`/`BLOB_READ_WRITE_TOKEN` missing; a cached-analysis reuse path asserting zero provider calls while still rendering; an AI-failure path asserting zero renderer calls; a render-failure path and a storage-failure path, each asserting no `status = 'published'` UPDATE was ever issued; a lost publish race mapping to `locked`; the lock always releasing even after an unexpected mid-flight error; source-inspection tests for the single `generateWeeklyAnalysisIfNeeded` call site and the single provider construction site; cron-route auth tests (missing header, wrong secret, unconfigured `CRON_SECRET`) and a no-leaked-detail source check; and source/config checks confirming `next.config.js`'s Chromium tracing entry and `vercel.json`'s cron shape.

Uses the same "fake Pool" technique as `tests/weekly-report-publish-service.test.cjs` (a plain object matching pg's `.query(sql, params)` shape, pattern-matching on SQL text), extended to also cover `generateWeeklyAnalysisIfNeeded()`'s own `analysis-repo.ts` queries -- including correctly mimicking that a real `jsonb` column round-trips `JSON.stringify()`d input back into a parsed object, a real bug this test file's own fake pool hit and fixed during authoring (storing the raw stringified parameter instead of parsing it back produced a `.split()`-on-undefined crash deep in the render-model builder, since the "assessment" the fake pool handed back was a JSON string, not an object). Combined with full dependency injection (`buildSnapshot`, `buildAnalystInput`, `provider`, `pdfRenderer`, `artifactStorage`), this suite exercises the real branching logic of `orchestrateWeeklyReport()`, `generateWeeklyAnalysisIfNeeded()`, and `publishWeeklyReportIfReady()` together with no live EIA, Anthropic, Chromium, Blob, or Postgres access anywhere in the suite.
...
Full JS suite after this phase: **1411 tests, 1330 pass, 0 fail, 81 skipped** (all DB-gated, unchanged standing limitation). `npm run typecheck`: clean. `npm run build`: clean; route table gained exactly `/api/cron/reports`. The Chromium bin-file tracing requirement was verified directly against the built output -- `.next/server/app/api/cron/reports/route.js.nft.json` lists all 4 required `@sparticuz/chromium/bin/*.br` files.
