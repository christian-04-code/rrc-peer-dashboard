...
- **7D.1 — Visual polish + serverless-Chromium compatibility check — DONE, see §29.9.** Fixed a real Chromium launch-arg bug, pinned Node/dependency versions for Vercel, three reference-inspired visual refinements. **A real live-Vercel invocation of the actual `@sparticuz/chromium` binary is still NOT confirmed** -- blocked by this project's own Vercel Deployment Protection; this is the single most important open item before 7F schedules anything real.
...
- **7F — Orchestration**: the actual gated cron (§17) — new `vercel.json` entry, `app/api/cron/reports/route.ts`, advisory-lock-guarded orchestration function that calls `runWeeklySnapshotBuild()`, `generateWeeklyAnalysisIfNeeded()` (§26.9), then `renderWeeklyReportPdf()` (§29.6) and (once 7E exists) carries a `ready` snapshot + assessment + PDF the rest of the way to `published`. **Before this phase schedules anything real: the user must run a real controlled invocation of the deployed Chromium renderer past this project's own Vercel Deployment Protection** (§29.9 -- the one thing 7D.1 could not safely complete itself), confirming the real binary launches, renders, and produces a valid PDF with an enforced page count. Also where the first real (paid) Anthropic call for this subsystem should happen, via the same controlled-Preview-validation pattern Phase 6 established (§26.10).
