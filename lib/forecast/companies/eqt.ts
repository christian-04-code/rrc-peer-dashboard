...
import {
  EQT_FORECAST_YEARS,
  EQT_LATEST_ACTUAL_PERIOD,
  EQT_VALUATION_PRESETS,
  ...
  runEqtAnnualForecast,
  type EqtForecastYear,
  type EqtAnnualForecastRequest,
  type EqtAnnualForecastResult
} from "@/lib/forecast/scenarios/eqt-annual";
import { eqtManagementGuidance } from "@/lib/forecast/guidance/eqt";
import { eqtLatestDetailedBaseline } from "@/lib/forecast/data/eqt-baseline";
import type { ResolvedAnnualValue } from "@/lib/forecast/scenarios/annual-shared";

const company = rawRegistry.companies.EQT;
const forecastYears = buildForecastYearWindow(EQT_LATEST_ACTUAL_PERIOD);
if (forecastYears.join(",") !== EQT_FORECAST_YEARS.join(",")) {
  throw new Error("EQT forecast periods are out of sync with the latest actual period.");
}

const available = { status: "available", warning: null } as const;
const hedgesUnavailable = {
  status: "unavailable",
  warning: "EQT hedge positions have not yet been ingested into this engine's structured hedge format; the Forecast model treats EQT as unhedged (pre-cash-settled-derivative realized pricing) pending a dedicated hedge-data backfill."
} as const;

export const EQT_FORECAST_METADATA: ForecastCompanyMetadata = {
  ticker: "EQT",
  name: company.name,
  ...
  periods: {
    latestActualPeriod: EQT_LATEST_ACTUAL_PERIOD,
    latestDetailedActualPeriod: EQT_LATEST_ACTUAL_PERIOD,
    forecastYears,
...
): Partial<Record<EqtForecastYear, T>> {
  const input = record(value, path);
  const output: Partial<Record<EqtForecastYear, T>> = {};
  for (const key of Object.keys(input)) {
    if (!EQT_FORECAST_YEARS.includes(key as EqtForecastYear)) invalid(`${path}.${key} is outside the active forecast window.`);
  }
...
function sanitizeRequest(bodyValue: ForecastScenarioBody): EqtAnnualForecastRequest {
  ...
  if (requestedForwardYear !== undefined && !EQT_FORECAST_YEARS.includes(requestedForwardYear as EqtForecastYear)) {
  ...
  const liveCommodity = record(body.liveCommodity, "liveCommodity") as EqtAnnualForecastRequest["liveCommodity"];
  ...
  return {
    ...
    liveCommodity,
    valuation: {
      targetEvToEbitdax: optionalNumber(valuation.targetEvToEbitdax, "valuation.targetEvToEbitdax", 0) ?? EQT_VALUATION_PRESETS.base,
      forwardYear: (requestedForwardYear as EqtForecastYear | undefined) ?? (EQT_FORECAST_METADATA.periods!.defaultForwardYear as EqtForecastYear),
...
function buildFieldProvenance(request: EqtAnnualForecastRequest): Record<string, ForecastFieldProvenance> {
  ...
  fields["valuation.targetEvToEbitdax"] = request.valuation.targetEvToEbitdax !== EQT_VALUATION_PRESETS.base ? "user_override" : "model";
...
function defaultRequest(): EqtAnnualForecastRequest {
  return {
    production: {},
    costs: {},
    capex: {},
    ...
    valuation: { targetEvToEbitdax: EQT_VALUATION_PRESETS.base, forwardYear: "2027" }
  };
}
...
function legacyDefaults(): Record<string, unknown> {
  return {
    latestActualPeriod: EQT_LATEST_ACTUAL_PERIOD,
    valuationPresets: EQT_VALUATION_PRESETS,
    guidance: eqtManagementGuidance,
    productionDefaults: Object.fromEntries(EQT_FORECAST_YEARS.map((year) => [year, resolveAnnualProductionDefault(year)])),
    commodityProductionDefaults: Object.fromEntries(EQT_FORECAST_YEARS.map((year) => [year, resolveAnnualCommodityProductionDefaults(year)])),
    costDefaults: Object.fromEntries(EQT_FORECAST_YEARS.map((year) => [year, resolveAnnualCostDefaults(year)])),
    capexDefaults: Object.fromEntries(EQT_FORECAST_YEARS.map((year) => [year, resolveAnnualCapexDefault(year)])),
    pricingDefaults: Object.fromEntries(EQT_FORECAST_YEARS.map((year) => [year, resolveAnnualPricingDefaults(year)])),
    currentNetDebtMillion: eqtLatestDetailedBaseline.netDebtMillion.value,
    dilutedSharesMillion: eqtLatestDetailedBaseline.dilutedSharesMillion.value,
    result: runEqtAnnualForecast(defaultRequest())
  };
}

export const eqtForecastAdapter: ForecastCompanyAdapter<EqtAnnualForecastResult> = {
  metadata: EQT_FORECAST_METADATA,
  configuration: {
    latestDetailedActualPeriod: EQT_LATEST_ACTUAL_PERIOD,
    guidance: eqtManagementGuidance,
    defaults: { production: available, pricing: available, costs: available, capex: available },
    balanceSheet: {
      netDebtMillion: eqtLatestDetailedBaseline.netDebtMillion.value,
      dilutedSharesMillion: eqtLatestDetailedBaseline.dilutedSharesMillion.value,
      status: available
    },
  ...
  runForecast(body) {
    ...
    return {
      company: EQT_FORECAST_METADATA,
      result: runEqtAnnualForecast(request),
