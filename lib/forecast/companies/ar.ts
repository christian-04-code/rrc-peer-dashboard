...
import {
  AR_FORECAST_YEARS,
  AR_LATEST_ACTUAL_PERIOD,
  AR_VALUATION_PRESETS,
  ...
  runArAnnualForecast,
  type ArForecastYear,
  type ArAnnualForecastRequest,
  type ArAnnualForecastResult
} from "@/lib/forecast/scenarios/ar-annual";
import { arManagementGuidance } from "@/lib/forecast/guidance/ar";
import { arLatestDetailedBaseline } from "@/lib/forecast/data/ar-baseline";
import type { ResolvedAnnualValue } from "@/lib/forecast/scenarios/annual-shared";

const company = rawRegistry.companies.AR;
const forecastYears = buildForecastYearWindow(AR_LATEST_ACTUAL_PERIOD);
if (forecastYears.join(",") !== AR_FORECAST_YEARS.join(",")) {
  throw new Error("AR forecast periods are out of sync with the latest actual period.");
}

const available = { status: "available", warning: null } as const;
const hedgesUnavailable = {
  status: "unavailable",
  warning: "AR hedge positions have not yet been ingested into this engine's structured hedge format; the Forecast model treats AR as unhedged (pre-hedge realized pricing) pending a dedicated hedge-data backfill."
} as const;

export const AR_FORECAST_METADATA: ForecastCompanyMetadata = {
  ticker: "AR",
  name: company.name,
  ...
  periods: {
    latestActualPeriod: AR_LATEST_ACTUAL_PERIOD,
    latestDetailedActualPeriod: AR_LATEST_ACTUAL_PERIOD,
    forecastYears,
...
): Partial<Record<ArForecastYear, T>> {
  const input = record(value, path);
  const output: Partial<Record<ArForecastYear, T>> = {};
  for (const key of Object.keys(input)) {
    if (!AR_FORECAST_YEARS.includes(key as ArForecastYear)) invalid(`${path}.${key} is outside the active forecast window.`);
  }
...
function sanitizeRequest(bodyValue: ForecastScenarioBody): ArAnnualForecastRequest {
  ...
  if (requestedForwardYear !== undefined && !AR_FORECAST_YEARS.includes(requestedForwardYear as ArForecastYear)) {
  ...
  const liveCommodity = record(body.liveCommodity, "liveCommodity") as ArAnnualForecastRequest["liveCommodity"];
  ...
  return {
    ...
    liveCommodity,
    valuation: {
      targetEvToEbitdax: optionalNumber(valuation.targetEvToEbitdax, "valuation.targetEvToEbitdax", 0) ?? AR_VALUATION_PRESETS.base,
      forwardYear: (requestedForwardYear as ArForecastYear | undefined) ?? (AR_FORECAST_METADATA.periods!.defaultForwardYear as ArForecastYear),
...
function buildFieldProvenance(request: ArAnnualForecastRequest): Record<string, ForecastFieldProvenance> {
  ...
  fields["valuation.targetEvToEbitdax"] = request.valuation.targetEvToEbitdax !== AR_VALUATION_PRESETS.base ? "user_override" : "model";
...
function defaultRequest(): ArAnnualForecastRequest {
  return {
    production: {},
    costs: {},
    capex: {},
    ...
    valuation: { targetEvToEbitdax: AR_VALUATION_PRESETS.base, forwardYear: "2027" }
  };
}
...
function legacyDefaults(): Record<string, unknown> {
  return {
    latestActualPeriod: AR_LATEST_ACTUAL_PERIOD,
    valuationPresets: AR_VALUATION_PRESETS,
    guidance: arManagementGuidance,
    productionDefaults: Object.fromEntries(AR_FORECAST_YEARS.map((year) => [year, resolveAnnualProductionDefault(year)])),
    commodityProductionDefaults: Object.fromEntries(AR_FORECAST_YEARS.map((year) => [year, resolveAnnualCommodityProductionDefaults(year)])),
    costDefaults: Object.fromEntries(AR_FORECAST_YEARS.map((year) => [year, resolveAnnualCostDefaults(year)])),
    capexDefaults: Object.fromEntries(AR_FORECAST_YEARS.map((year) => [year, resolveAnnualCapexDefault(year)])),
    pricingDefaults: Object.fromEntries(AR_FORECAST_YEARS.map((year) => [year, resolveAnnualPricingDefaults(year)])),
    currentNetDebtMillion: arLatestDetailedBaseline.netDebtMillion.value,
    dilutedSharesMillion: arLatestDetailedBaseline.dilutedSharesMillion.value,
    result: runArAnnualForecast(defaultRequest())
  };
}

export const arForecastAdapter: ForecastCompanyAdapter<ArAnnualForecastResult> = {
  metadata: AR_FORECAST_METADATA,
  configuration: {
    latestDetailedActualPeriod: AR_LATEST_ACTUAL_PERIOD,
    guidance: arManagementGuidance,
    defaults: { production: available, pricing: available, costs: available, capex: available },
    balanceSheet: {
      netDebtMillion: arLatestDetailedBaseline.netDebtMillion.value,
      dilutedSharesMillion: arLatestDetailedBaseline.dilutedSharesMillion.value,
      status: available
    },
  ...
  runForecast(body) {
    ...
    return {
      company: AR_FORECAST_METADATA,
      result: runArAnnualForecast(request),
