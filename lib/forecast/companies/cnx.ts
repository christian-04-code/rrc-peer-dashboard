...
import {
  CNX_FORECAST_YEARS,
  CNX_LATEST_ACTUAL_PERIOD,
  CNX_VALUATION_PRESETS,
  ...
  runCnxAnnualForecast,
  type CnxForecastYear,
  type CnxAnnualForecastRequest,
  type CnxAnnualForecastResult
} from "@/lib/forecast/scenarios/cnx-annual";
import { cnxManagementGuidance } from "@/lib/forecast/guidance/cnx";
import { cnxLatestDetailedBaseline } from "@/lib/forecast/data/cnx-baseline";
import type { ResolvedAnnualValue } from "@/lib/forecast/scenarios/annual-shared";

const company = rawRegistry.companies.CNX;
const forecastYears = buildForecastYearWindow(CNX_LATEST_ACTUAL_PERIOD);
if (forecastYears.join(",") !== CNX_FORECAST_YEARS.join(",")) {
  throw new Error("CNX forecast periods are out of sync with the latest actual period.");
}

const available = { status: "available", warning: null } as const;
const hedgesUnavailable = {
  status: "unavailable",
  warning: "CNX hedge positions have not yet been ingested into this engine's structured hedge format; the Forecast model treats CNX as unhedged (pre-hedge realized pricing) pending a dedicated hedge-data backfill."
} as const;

export const CNX_FORECAST_METADATA: ForecastCompanyMetadata = {
  ticker: "CNX",
  name: company.name,
  ...
  periods: {
    latestActualPeriod: CNX_LATEST_ACTUAL_PERIOD,
    latestDetailedActualPeriod: CNX_LATEST_ACTUAL_PERIOD,
    forecastYears,
...
): Partial<Record<CnxForecastYear, T>> {
  const input = record(value, path);
  const output: Partial<Record<CnxForecastYear, T>> = {};
  for (const key of Object.keys(input)) {
    if (!CNX_FORECAST_YEARS.includes(key as CnxForecastYear)) invalid(`${path}.${key} is outside the active forecast window.`);
  }
...
function sanitizeRequest(bodyValue: ForecastScenarioBody): CnxAnnualForecastRequest {
  ...
  if (requestedForwardYear !== undefined && !CNX_FORECAST_YEARS.includes(requestedForwardYear as CnxForecastYear)) {
  ...
  const liveCommodity = record(body.liveCommodity, "liveCommodity") as CnxAnnualForecastRequest["liveCommodity"];
  ...
  return {
    ...
    liveCommodity,
    valuation: {
      targetEvToEbitdax: optionalNumber(valuation.targetEvToEbitdax, "valuation.targetEvToEbitdax", 0) ?? CNX_VALUATION_PRESETS.base,
      forwardYear: (requestedForwardYear as CnxForecastYear | undefined) ?? (CNX_FORECAST_METADATA.periods!.defaultForwardYear as CnxForecastYear),
...
function buildFieldProvenance(request: CnxAnnualForecastRequest): Record<string, ForecastFieldProvenance> {
  ...
  fields["valuation.targetEvToEbitdax"] = request.valuation.targetEvToEbitdax !== CNX_VALUATION_PRESETS.base ? "user_override" : "model";
...
function defaultRequest(): CnxAnnualForecastRequest {
  return {
    production: {},
    costs: {},
    capex: {},
    ...
    valuation: { targetEvToEbitdax: CNX_VALUATION_PRESETS.base, forwardYear: "2027" }
  };
}
...
function legacyDefaults(): Record<string, unknown> {
  return {
    latestActualPeriod: CNX_LATEST_ACTUAL_PERIOD,
    valuationPresets: CNX_VALUATION_PRESETS,
    guidance: cnxManagementGuidance,
    productionDefaults: Object.fromEntries(CNX_FORECAST_YEARS.map((year) => [year, resolveAnnualProductionDefault(year)])),
    commodityProductionDefaults: Object.fromEntries(CNX_FORECAST_YEARS.map((year) => [year, resolveAnnualCommodityProductionDefaults(year)])),
    costDefaults: Object.fromEntries(CNX_FORECAST_YEARS.map((year) => [year, resolveAnnualCostDefaults(year)])),
    capexDefaults: Object.fromEntries(CNX_FORECAST_YEARS.map((year) => [year, resolveAnnualCapexDefault(year)])),
    pricingDefaults: Object.fromEntries(CNX_FORECAST_YEARS.map((year) => [year, resolveAnnualPricingDefaults(year)])),
    currentNetDebtMillion: cnxLatestDetailedBaseline.netDebtMillion.value,
    dilutedSharesMillion: cnxLatestDetailedBaseline.dilutedSharesMillion.value,
    result: runCnxAnnualForecast(defaultRequest())
  };
}

export const cnxForecastAdapter: ForecastCompanyAdapter<CnxAnnualForecastResult> = {
  metadata: CNX_FORECAST_METADATA,
  configuration: {
    latestDetailedActualPeriod: CNX_LATEST_ACTUAL_PERIOD,
    guidance: cnxManagementGuidance,
    defaults: { production: available, pricing: available, costs: available, capex: available },
    balanceSheet: {
      netDebtMillion: cnxLatestDetailedBaseline.netDebtMillion.value,
      dilutedSharesMillion: cnxLatestDetailedBaseline.dilutedSharesMillion.value,
      status: available
    },
  ...
  runForecast(body) {
    ...
    return {
      company: CNX_FORECAST_METADATA,
      result: runCnxAnnualForecast(request),
