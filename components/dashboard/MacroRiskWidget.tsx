...
/**
 * Splits AI-generated prose into one paragraph per sentence for a scannable
 * vertical layout, instead of one dense block. Purely mechanical (splits on
 * sentence-ending punctuation followed by a capitalized word) -- never
 * reorders, summarizes, categorizes, or drops any sentence, so it can never
 * misrepresent the underlying AI output. A decimal like "3.7%" is never
 * split on since no capital letter follows it.
 */
        ...
        <div className="macro-risk-side">
          ...
          <div className="macro-ai-summary">
            ...
            {(data.aiSummaryStatus === "ready" || data.aiSummaryStatus === "stale") && data.aiSummary ? (
              <>
                {splitSentences(data.aiSummary.summary).map((sentence, index) => (
                  <p className="macro-ai-summary-text" key={index}>{highlightNumbers(sentence)}</p>
                ))}
                <small>
