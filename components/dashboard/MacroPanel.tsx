...
// Grouped for the topic nav (Section 4): three conceptual clusters -- the six
// live EIA fundamentals topics, EIA's own forward projection (a genuinely
// different data class: forecast, not observation), and Rigs (Baker Hughes,
// manually imported -- a different data class again). Grouping is presentation
// only; it changes nothing about which topic is selected or how its data loads.
const TOPIC_GROUPS: { label: string; topics: { id: Topic; label: string }[] }[] = [
  {
    label: "Fundamentals",
    topics: [
  ...
  { label: "Outlook", topics: [{ id: "eia-outlook", label: "EIA Outlook" }] },
  { label: "Activity", topics: [{ id: "rigs", label: "Rigs" }] }
];
...
type DataHealth = { label: string; tone: "positive" | "caution" | "negative"; detail: string };

/**
 * Derived entirely from status/freshness fields the data pipeline already
 * computes (NormalizedMarketMetric.status/.freshness, each MacroFundamentals
 * subsystem's .status, MacroSteoResponse.status) -- no new business logic,
 * no invented health category. "Lagged" is treated as healthy: it is EIA's
 * own normal publication cadence (confirmed in production), not a fault, so
 * counting it would make "Healthy" essentially unreachable and train users to
 * ignore the indicator. Only a genuine fetch failure (.status "unavailable",
 * see PulseMetric's hasError) or a "stale" observation (older than even the
 * lagged threshold) downgrades the badge.
 */
function computeDataHealth(params: {
  metrics: NormalizedMarketMetric[];
  fundamentals: MacroFundamentalsResponse | null;
  steoStatus: "ok" | "unavailable" | undefined;
}): DataHealth {
  const failed: string[] = params.metrics.filter((metric) => metric.status !== "ok").map((metric) => metric.label);
  if (params.fundamentals) {
    if (params.fundamentals.storage.status !== "ok") failed.push("Regional storage");
    if (params.fundamentals.production.status !== "ok") failed.push("State production");
    if (params.fundamentals.demand.status !== "ok") failed.push("Demand by sector");
  }
  if (params.steoStatus === "unavailable") failed.push("EIA STEO outlook");
  const stale = params.metrics.filter((metric) => metric.freshness === "stale").map((metric) => metric.label);

  if (failed.length > 0) {
    return {
      label: failed.length === 1 ? "1 source unavailable" : `${failed.length} sources unavailable`,
      tone: "negative",
      detail: `Fetch failed: ${failed.join(", ")}.`
    };
  }
  if (stale.length > 0) {
    return {
      label: stale.length === 1 ? "1 source stale" : `${stale.length} sources stale`,
      tone: "caution",
      detail: `Older than the normal publication cadence: ${stale.join(", ")}.`
    };
  }
  return { label: "Healthy", tone: "positive", detail: "Every live EIA/OilPriceAPI/STEO source returned data within its normal publication cadence. Rigs is manually imported and reported separately below." };
}

/** Maps a metric's existing status/freshness fields to the Data Sources & Freshness table's status column -- no state is invented beyond what NormalizedMarketMetric already carries. */
function metricStatusLabel(metric: NormalizedMarketMetric): { label: string; tone: "current" | "lagged" | "stale" | "error" } {
  if (metric.status !== "ok") return { label: "Source error", tone: "error" };
  if (metric.freshness === "stale") return { label: "Stale", tone: "stale" };
  if (metric.freshness === "lagged") return { label: "Lagged", tone: "lagged" };
...
export function MacroPanel() {
  ...
  const dataHealth = computeDataHealth({ metrics, fundamentals: fundamentals.data, steoStatus: steo.data?.status });
  ...
  const regimeTone = gasBalance.gasState === "Tightening" ? "positive" : gasBalance.gasState === "Loosening" ? "negative" : "neutral";
  const regimeExplanation = `Storage is ${formatPct(gasBalance.storagePct)} versus its five-year average (${gasBalance.storageState}); LNG exports are ${formatPct(gasBalance.lngYoY)} year over year (${gasBalance.lngState}). Tightening requires storage at least 5% below normal and LNG exports growing at least 5% YoY; loosening requires the inverse.`;

  return (
    <div className="macro-panel">
      <header className="macro-page-head-v2">
        <div className="macro-page-title">
          <h1>Natural Gas &amp; NGL Intelligence</h1>
          <p>U.S. gas fundamentals, Appalachia supply, LNG, demand and EIA outlook.</p>
        </div>
        <div className="macro-trust-row">
          <div className="macro-trust-card">
            <span>LAST UPDATED</span>
            <strong>{macroRisk.loading ? "Loading…" : formatRefreshTimestamp(macroRisk.data?.lastOrchestrationAt)}</strong>
            <small>Most recent successful Macro refresh · sources update on different schedules</small>
          </div>
          <div className="macro-trust-card">
            <span>LIVE DATA HEALTH</span>
            <strong className={`macro-health ${dataHealth.tone}`}><i className={`freshness-dot ${dataHealth.tone === "positive" ? "current" : dataHealth.tone === "caution" ? "lagged" : "error"}`} />{dataHealth.label}</strong>
            <small>{dataHealth.detail}</small>
          </div>
        </div>
        {market.error ?? fundamentals.error ? <p className="macro-page-error">{market.error ?? fundamentals.error}</p> : null}
      </header>

      <section className="macro-section macro-exec-summary">
        <SectionHeader eyebrow="MARKET CONDITION" title="What's happening in the U.S. gas market" description="Deterministic classification from storage deviation and LNG export growth -- the same rule the Macro Snapshot evidence panel uses." />
        <div className="macro-exec-body">
          <div className={`macro-exec-regime ${regimeTone}`}>
            <span>NATIONAL GAS BALANCE</span>
            <strong>{gasBalance.gasState}</strong>
            <p>{regimeExplanation}</p>
            <small>A Tightening read (low storage + growing LNG demand) is directionally supportive for Range&apos;s gas price realizations; Loosening is directionally unsupportive.</small>
          </div>
          <div className="macro-exec-signals">
            ...
            <Stat label="Power demand YoY" value={formatPct(monthlyYoy(electricPower?.history ?? []))} note={observationLabel(electricPower?.period, "monthly", electricPower?.freshness)} />
      ...
      <section className="macro-section macro-pulse">
        <SectionHeader eyebrow="MARKET PULSE" title="Live / latest market indicators" description="Real-time current price where available, otherwise the latest official EIA observation for its own reporting period. Not the full Macro dataset -- see the topics below for storage detail, Appalachia, demand and EIA's forward outlook." />
      ...
      <nav className="macro-topic-groups" aria-label="Macro topic">
        {TOPIC_GROUPS.map((group) => (
          <div className="macro-topic-group" key={group.label}>
            <span className="macro-topic-group-label">{group.label}</span>
            <div className="macro-segmented macro-topic-tabs">
      ...
      {topic === "gas-balance" ? (
        <section className="macro-section macro-snapshot-section">
          <SectionHeader eyebrow="EVIDENCE" title="Every driver behind the Gas Balance classification" description="The National Gas Balance read itself is above, in the Market Condition summary. This is the full deterministic evidence set it and the other topic classifications are built from -- select a row for its exact rule and inputs." asOf={`Storage ${storageMetric?.period ? formatWeekEnding(storageMetric.period) : "--"}${storageMetric?.freshness === "stale" ? " (stale)" : ""} · LNG ${lngMetric?.period ? formatDataDate(lngMetric.period) : "--"}${lngMetric?.freshness === "stale" ? " (stale)" : ""}`} />
      ...
      {topic === "storage" ? (
        <section className="macro-section macro-storage-section">
          ...
          {forecastSeries("workingGasStorageForecast", "#e5ad63") ? (
            <>
              <div className="macro-subsection-head compact"><div><span className="macro-forecast-eyebrow">EIA STEO OUTLOOK · FORECAST</span><h3>Working gas storage forecast</h3></div><p>{steoVintageLabel("workingGasStorageForecast")}</p></div>
      ...
      {topic === "appalachia" ? (
        <section className="macro-section">
          ...
          <div className="macro-primary-chart emphasis">
            <div className="macro-card-title"><div><h3>PA + WV + OH marketed production</h3><span>{observationLabel(appalachia.period, "monthly")} · Monthly · U.S. EIA</span></div><strong className="macro-emphasis-value">{appalachia.current === null ? "--" : new Intl.NumberFormat("en-US").format(appalachia.current)} <small>MMcf/month</small></strong></div>
          ...
          <div className="macro-connector"><span>RANGE IMPACT</span><p>What the Appalachia and national data above mean for Range, ranked by the same deterministic engine.</p></div>
      ...
      {topic === "demand" ? (
        <section className="macro-grid-row macro-grid-row-demand">
          <article className="macro-section macro-grid-card">
            ...
            <div className="macro-subsection-head compact"><div><span className="macro-forecast-eyebrow">EIA STEO OUTLOOK · FORECAST</span><h3>Electric power demand forecast</h3></div><p>{steoVintageLabel("electricPowerConsumptionForecast")}</p></div>
            ...
            <div className="macro-subsection-head compact"><div><span className="macro-forecast-eyebrow">EIA STEO OUTLOOK · FORECAST</span><h3>Industrial demand: actual vs. forecast</h3></div><p>{steoVintageLabel("industrialConsumptionForecast")}</p></div>
      ...
      {topic === "eia-outlook" ? (
        <section className="macro-section macro-outlook-section">
          <SectionHeader eyebrow="EIA OUTLOOK · FORECAST" title="Short-Term Energy Outlook" description="EIA's own forward projection, actual-vs-forecast where an observed counterpart exists, with forecast revisions once a second monthly snapshot has been captured. This topic is forecast data -- everything elsewhere under Fundamentals is an observed actual." />
      ...
      {topic === "rigs" ? (
        <section className="macro-section">
          ...
          <div className="macro-manual-badge">
            <i className="freshness-dot lagged" />
            <div><strong>Manually imported data -- not on the live EIA pipeline</strong><span>{formatWeekEnding(getRigDataset().source.reportDate)} · refreshed by running the Baker Hughes import script, not by the daily Macro cron</span></div>
          </div>
      ...
      <details className="macro-freshness-details">
        <summary>
          ...
          <span>Observation period and retrieval timestamp are tracked separately per source; publication weekdays are not assumed.</span>
        </summary>
        <div className="macro-freshness-table" role="table" aria-label="Data sources and freshness">
          <div className="macro-freshness-row header" role="row"><span>Source</span><span>Observation</span><span>Retrieved</span><span>Frequency</span><span>Status</span></div>
          {metrics.map((metric) => {
            const status = metricStatusLabel(metric);
            return (
              <div className="macro-freshness-row" role="row" key={metric.id}>
                <span>EIA · {metric.label}</span>
                <span>{observationLabel(metric.period, metric.frequency)}</span>
                <span>{formatRefreshTimestamp(metric.fetchedAt)}</span>
                <span>{metric.frequency}</span>
                <span className={`macro-freshness-status ${status.tone}`}><i className={`freshness-dot ${status.tone}`} />{status.label}</span>
              </div>
            );
          })}
          {currentQuotes ? Object.values(currentQuotes).map((quote) => (
            <div className="macro-freshness-row" role="row" key={quote.id}>
              <span>OilPriceAPI · {quote.label}</span>
              <span>Real-time</span>
              <span>{formatRefreshTimestamp(quote.asOf)}</span>
              <span>current market</span>
              <span className={`macro-freshness-status ${quote.status === "ok" ? "current" : "error"}`}><i className={`freshness-dot ${quote.status === "ok" ? "current" : "error"}`} />{quote.status === "ok" ? "Current" : "Source error"}</span>
            </div>
          )) : null}
          <div className="macro-freshness-row" role="row">
            <span>EIA · regional storage</span>
            <span>{observationLabel(east?.period, "weekly")}</span>
            <span>{formatRefreshTimestamp(fundamentals.data?.generatedAt)}</span>
            <span>weekly</span>
            <span className={`macro-freshness-status ${east?.freshness ?? "error"}`}><i className={`freshness-dot ${east?.freshness ?? "error"}`} />{east?.freshness === "current" ? "Current" : east?.freshness === "stale" ? "Stale" : east?.freshness === "lagged" ? "Lagged" : "Source error"}</span>
          </div>
          <div className="macro-freshness-row" role="row">
            <span>EIA STEO · Outlook (forecast)</span>
            <span>{steoObservation}</span>
            <span>{steo.data?.generatedAt ? formatRefreshTimestamp(steo.data.generatedAt) : "--"}</span>
            <span>monthly</span>
            <span className={`macro-freshness-status ${steo.data?.status === "ok" ? "current" : "error"}`}><i className={`freshness-dot ${steo.data?.status === "ok" ? "current" : "error"}`} />{steo.data?.status === "ok" ? "Current" : "Source error"}</span>
          </div>
          <div className="macro-freshness-row" role="row">
            <span>Baker Hughes · Rigs</span>
            <span>{formatWeekEnding(getRigDataset().source.reportDate)}</span>
            <span>Manual import</span>
            <span>weekly (manual)</span>
            <span className="macro-freshness-status manual"><i className="freshness-dot manual" />Manual</span>
          </div>
        </div>
      </details>
    </div>
